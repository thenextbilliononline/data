...
poverty <- cbind(poverty$`GEO.display-label`, poverty$HC02_EST_VC01)
poverty <- as.data.frame(poverty)
poverty <- poverty[-1,]
colnames(poverty) <- c("State", "Population.Below.Poverty.Line")
poverty
```
...
colnames(sex.and.age) <- c("State", "Median.Age", "Males.Per.100.Females")
sex.and.age
...
race
race <- cbind(race$`GEO.display-label`, race$HD01_VD02, race$HD01_VD03, race$HD01_VD04, race$HD01_VD05, race$HD01_VD07, race$HD01_VD10)
race <- race[-1,]
race <- as.data.frame(race, header = TRUE)
race <- race[-1,]
colnames(race) <- c("State", 
...
colnames(education.type) <- c("State", "STEM.education")
education.type
...
employment.type <- cbind(employment.type$`GEO.display-label`, employment.type$HC01_EST_VC03)
employment.type <- as.data.frame(employment.type)
employment.type <- employment.type[-1,]
colnames(employment.type) <- c("State", "Population.In.Service.Industry")
employment.type
...
education.level
education.level <- cbind(education.level$`GEO.display-label`, education.level$HC01_EST_VC03,  education.level$HC02_EST_VC11, education.level$	
HC01_EST_VC12,  education.level$HC01_EST_VC14, education.level$HC01_EST_VC15)
education.level <- as.data.frame(education.level)
education.level <- education.level[-1,]
colnames(education.level) <- c("State", "Less.Than.HS", "HS", "Some.College", "Bachelors", "Graduate")

```

```{r}
limited.english.households <- as.data.frame(read_csv("ACS_17_1YR_S1602_with_ann.csv"), header = TRUE)
limited.english.households <- cbind(limited.english.households$`GEO.display-label`, limited.english.households$HC03_EST_VC01)
...
colnames(limited.english.households) <- c("State", "Limited.English.Households")
...
df <- merge(df, geo.mobility, all = TRUE, by = "State")
