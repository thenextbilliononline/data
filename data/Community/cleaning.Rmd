...
race[11,]
race$White <- as.numeric(race$White)/state.pops
race$Black <- as.numeric(race$Black)/state.pops
race$Native <- as.numeric(race$Native)/state.pops
race$Other <- as.numeric(race$Other)/state.pops
race$Asian <- as.numeric(race$Asian)/state.pops
race$Two.or.more <- as.numeric(race$Two.or.more)/state.pops
race[11,3:8] <-race[11,3:8]/100
race
```
...
colnames(geo.mobility) <- c("State", "Population.Moved.In.Last.Year")
as.numeric(geo.mobility$Population.Moved.In.Last.Year) / (state.pops)
...
```{r}
indx <- grepl('Population' , colnames(df))
new.df <- df[!indx]
pops <- df[indx]
as.numeric(pops$Population)
divide.by.population <- function(x){
  x <- as.numeric(x)/as.numeric(pops$Population)
}
pops <- sapply(pops, divide.by.population)
pops
#pops
