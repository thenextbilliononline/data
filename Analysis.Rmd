...
```{r socio-economic-plots, echo=T, eval=T, message=F, fig.align="center"}
...
lit.gdp <- ggplot(annual.means, aes(x=lit, y=gdp)) + geom_point(size=2.5)
gdp.gini <- ggplot(annual.means, aes(x=gdp, y=gini)) + geom_point(size=2.5)
gini.lit <- ggplot(annual.means, aes(x=gini, y=lit)) + geom_point(size=2.5)
plot_grid(lit.gdp, gdp.gini, gini.lit, labels = c('1', '2', '3'))
```
