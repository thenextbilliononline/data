...
```{r lit-gdp, echo=T, eval=T, message=F, fig.align="right"}
plot(lit.annual~gdp.annual)
lit.gdp <- lm(lit.annual~gdp.annual)
summary(lit.gdp)
```
Weak, downward sloping correlation.

```{r gdp-gini, echo=T, eval=T, message=F}
plot(gdp.annual~gini.annual)
gdp.gini <- lm(gdp.annual~gini.annual)
summary(model)
```

Weak, downward sloping correlation.

```{r gini-lit, echo=T, eval=T, message=F}
plot(gini.annual~lit.annual)
gini.lit <- lm(gini.annual~lit.annual)
summary(gini.lit)
```
